...
def parseBLASTIteration(iteration, banyak_record):
    ...
    for hit in iteration.findall(".//Hit")[:banyak_record]:
        hitDict = dict()
        for k in ["Hit_id","Hit_def", "Hit_accession", "Hit_len"]:
...
def parseSingleIteration(tree, banyak_record):
    ...
    hitList = parseBLASTIteration(iteration, banyak_record)
    return hitList
...
def tampilHitList(hitList, withaccession = True):
...
dir_kerja = os.getcwd()
lapBLAST = r"%s\Hasil BLAST\BLAST naja.xml" % (dir_kerja)
XMLtreeBLAST = ElementTree.parse(lapBLAST)
hitL = parseSingleIteration(XMLtreeBLAST, 5)
tampilHitList(hitL[:5])