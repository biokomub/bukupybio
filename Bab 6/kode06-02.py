...
CLUSTALW_result = r"%s\Hasil Alignment\CLUSTAL\Aln 16S Bacteria.fasta" % (dir_kerja)
...
if os.path.isdir(CLUSTALW_result_dir) == False:
    ...
    print("Folder", CLUSTALW_result_dir , "harus dibuat...")
    pathlib.Path(CLUSTALW_result_dir).mkdir(parents = True)
    print("Folder", CLUSTALW_result_dir , "sudah dibuat!")
else:
    print("Folder", CLUSTALW_result_dir , "sudah ada...")
    pass

if not os.path.isfile(CLUSTALW_result):
    CLUSTALW_exe = r"D:\BIOTOOLS\ClustalW2\clustalw2.exe"
    cwline = ClustalwCommandline(CLUSTALW_exe, infile = FASTA_file, outfile = CLUSTALW_result, output = "fasta")
